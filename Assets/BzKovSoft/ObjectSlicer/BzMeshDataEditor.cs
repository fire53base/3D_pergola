...
namespace BzKovSoft.ObjectSlicer
{
	...
	{
		private const decimal MinPointDistanceSqr = 0.000001m;//0.00000000000000000000000001m;
		private const decimal MinPointAngle = 0.00001m;
		...
		public List<LinkedLoop<int>> GetEdgeLoops()
		{
			...
			while (parrentListsNode != null)
			{
				...
				if (_skipIfNotClosed)
				{
					...
					if (Convert.ToDecimal((firstValue - lastValue).sqrMagnitude) < MinPointDistanceSqr)
					{
						continue;
					}
				}
				...
				while (counter != 0)
				{
					...
					if (Convert.ToDecimal((prevValue - v).sqrMagnitude) < MinPointDistanceSqr)
					{
						// delete same positions
						current.Remove();
					}
					else if( Convert.ToDecimal(Vector3.Angle(prevValue - prevValue2, v - prevValue)) < MinPointAngle)
					{
		...
		public void EdgeLoops_JoinBySameValue(LinkedList<LinkedLoop<int>> edgeLoops)
		{
			...
			while (parrentListsNode != null)
			{
				...
				while (joinCandidate != null)
				{
					...
					if (Convert.ToDecimal((listLastValue - restFirstValue).sqrMagnitude) < MinPointDistanceSqr)
					{
					...
					else if (Convert.ToDecimal((listFirstValue - restLastValue).sqrMagnitude) < MinPointDistanceSqr)
					{
