...
namespace RESTfulHTTPServer.src.controller
{
    public class RoutingManager
    {
        ...
        /// <returns></returns>
		public async Task<Response> CallDelegater(Request request)
        {
            Response response;
            try
            {
                ...
                object clObj = c.Invoke(new object[] { });
                MethodInfo method = t.GetMethod(request.GetRoute().GetInvokerMethod());
                response = await (Task<Response>)method.Invoke(clObj, new object[] { request });
            }
            catch (Exception e)
            {
